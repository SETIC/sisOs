...
qq.DragAndDrop=function(a){function b(){m===j&&!i&&(f.callbacks.log("Grabbed "+k.length+" files after tree traversal."),h.dropDisabled(!1),f.callbacks.dropProcessing(!1,k))}function c(a){var d;m+=1;a.isFile?a.file(function(a){k.push(a);j+=1;b()}):a.isDirectory&&(i=!0,a=a.createReader(),a.readEntries(function(a){j+=1;for(d=0;d<a.length;d+=1)c(a[d]);i=!1;a.length||b()}))}function d(a){h=new qq.UploadDropZone({element:a,onEnter:function(b){qq(a).addClass(f.classes.dropActive);b.stopPropagation()},onLeaveNotDescendants:function(){qq(a).removeClass(f.classes.dropActive)},
onDrop:function(d){f.hideDropzones&&qq(a).hide();qq(a).removeClass(f.classes.dropActive);var d=d.dataTransfer,e,g;f.callbacks.dropProcessing(!0);h.dropDisabled(!0);if(1<d.files.length&&!f.multiple)f.callbacks.error("tooManyFilesError","");else if(k=[],j=m=0,qq.isFolderDropSupported(d)){e=d.items;for(d=0;d<e.length;d+=1)if(g=e[d].webkitGetAsEntry())g.isFile?(k.push(e[d].getAsFile()),d===e.length-1&&b()):c(g)}else f.callbacks.dropProcessing(!1,d.files),h.dropDisabled(!1)}});l.addDisposer(function(){h.dispose()});
f.hideDropzones&&qq(a).hide()}function e(a){var b;qq.each(a.dataTransfer.types,function(a,c){if("Files"===c)return b=!0,!1});return b}function g(){f.dropArea&&f.extraDropzones.push(f.dropArea);var a,b=f.extraDropzones;for(a=0;a<b.length;a+=1)d(b[a]);f.dropArea&&(!qq.ie()||qq.ie10())&&l.attach(document,"dragenter",function(c){if(!h.dropDisabled()&&e(c)&&!qq(f.dropArea).hasClass(f.classes.dropDisabled)){f.dropArea.style.display="block";for(a=0;a<b.length;a+=1)b[a].style.display="block"}});l.attach(document,
"dragleave",function(c){if(f.hideDropzones&&qq.FineUploader.prototype._leaving_document_out(c))for(a=0;a<b.length;a+=1)qq(b[a]).hide()});l.attach(document,"drop",function(c){if(f.hideDropzones)for(a=0;a<b.length;a+=1)qq(b[a]).hide();c.preventDefault()})}var f,h,i,k=[],m=0,j=0,l=new qq.DisposeSupport;f={dropArea:null,extraDropzones:[],hideDropzones:!0,multiple:!0,classes:{dropActive:null},callbacks:{dropProcessing:function(){},error:function(){},log:function(){}}};qq.extend(f,a);return{setup:function(){g()},
setupExtraDropzone:function(a){f.extraDropzones.push(a);d(a)},removeExtraDropzone:function(a){var b,c=f.extraDropzones;for(b in c)if(c[b]===a)return c.splice(b,1)},dispose:function(){l.dispose();h.dispose()}}};
